...
use ax_driver::{DriverGeneric, PlatformDevice, net::PlatformDeviceNet, probe::OnProbeError};
use ax_kspin::SpinNoIrq;
use ax_plat::mem::{pa, phys_to_virt, va, virt_to_phys};
use rd_net::{DmaBuffer, Event, IRxQueue, ITxQueue, Interface, NetError, QueueConfig};

use crate::{
    config::devices::{GMAC0_IRQ, GMAC0_PADDR},
    mem::PHYS_UNCACHED_OFFSET,
};
...
impl GmacRegs {
    fn new(paddr: usize) -> Self {
        let base = phys_to_virt(pa!(paddr)).as_mut_ptr();
        Self {
...
impl GmacNet {
    fn new() -> Result<Self, GmacError> {
        let regs = GmacRegs::new(GMAC0_PADDR);
        let version = regs.mac.read(GMAC_VERSION);
        let config = regs.mac.read(GMAC_CONFIG);

        debug!(
            "{DEVICE_NAME}: probe paddr={:#x}, version={:#x}, config={:#x}",
            GMAC0_PADDR, version, config
        );
        ...
        log_link_state(regs.link_state());

        regs.stop_tx_rx();
        regs.disable_irq();
        regs.reset_dma()?;
        regs.set_mac_address(DEFAULT_MAC_ADDRESS);
        ...
        info!(
            ...
            DEFAULT_MAC_ADDRESS[0],
            DEFAULT_MAC_ADDRESS[1],
            DEFAULT_MAC_ADDRESS[2],
            DEFAULT_MAC_ADDRESS[3],
            DEFAULT_MAC_ADDRESS[4],
            DEFAULT_MAC_ADDRESS[5],
        );
        debug!(
    ...
    fn handle_irq(&mut self) -> Event {
        ...
        if status & GMAC_LINE_INTF_INTR != 0 {
            ...
            if inner.regs.mac.read(GMAC_INTERRUPT_STATUS) & MAC_RGMII_INT_STATUS != 0 {
                log_link_state(inner.regs.link_state());
            }
        }
...
impl GmacState {
    ...
    fn maybe_start_rx(&mut self) -> Option<(usize, u32)> {
        if self.rx_started || self.rx_submitted < RX_START_THRESHOLD {
            return None;
        }
...
impl IRxQueue for GmacRxQueue {
    ...
    fn submit(&mut self, buffer: DmaBuffer) -> Result<(), NetError> {
        ...
        let rx_bus_addr = dma_addr32_net(rx_buf)?;
        let length =
...
static mut GMAC0_RING: AlignedGmacRing = AlignedGmacRing(GmacRing::new());

#[unsafe(link_section = ".dma_uncached")]
static mut GMAC0_BUFFERS: GmacBuffers = GmacBuffers::new();

fn probe_static(plat_dev: PlatformDevice) -> Result<(), OnProbeError> {
    let mut dev = GmacNet::new().map_err(|err| {
        OnProbeError::other(alloc::format!("failed to init {DEVICE_NAME}: {err}"))
    })?;
    dev.enable_irq_source();
    plat_dev.register_net(DEVICE_NAME, dev, Some(GMAC0_IRQ));
    info!(
        "registered {DEVICE_NAME} network device on irq {}",
        GMAC0_IRQ
    );
    Ok(())
}
...
fn ring_ptrs() -> RingPtrs {
    unsafe {
        let ring = addr_of_mut!(GMAC0_RING.0);
...
fn buffer_ptrs() -> BufferPtrs {
    unsafe {
        let buffers = addr_of_mut!(GMAC0_BUFFERS);
...
fn uncached_alias<T>(ptr: *mut T) -> *mut T {
    let paddr = virt_to_phys(va!(ptr as usize)).as_usize();
    (paddr + PHYS_UNCACHED_OFFSET) as *mut T
}

fn dma_addr32(ptr: *const u8, name: &'static str) -> Result<u32, GmacError> {
    let paddr = virt_to_phys(va!(ptr as usize)).as_usize() as u64;
