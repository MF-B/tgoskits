...
pub(crate) const PHYS_UNCACHED_OFFSET: usize = 0x8000_0000_0000_0000;

struct MemIfImpl;

fn is_mmio_paddr(paddr: PhysAddr) -> bool {
    let paddr = paddr.as_usize();
    MMIO_RANGES
        .iter()
        .any(|&(base, size)| paddr >= base && paddr < base.saturating_add(size))
}
...
impl MemIf for MemIfImpl {
    ...
    fn phys_to_virt(paddr: PhysAddr) -> VirtAddr {
        if is_mmio_paddr(paddr) {
            va!(paddr.as_usize() + PHYS_UNCACHED_OFFSET)
        } else {
    ...
    fn virt_to_phys(vaddr: VirtAddr) -> PhysAddr {
        ...
        } else if vaddr & 0xffff_0000_0000_0000 == PHYS_UNCACHED_OFFSET {
            pa!(vaddr - PHYS_UNCACHED_OFFSET)
        } else {
