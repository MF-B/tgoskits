...
/// Initializes the Loongson RTC and uses its TOY counter to provide wall time.
#[cfg(feature = "rtc")]
fn init_rtc() {
    ...
    const RTC_PADDR: PhysAddr = pa!(crate::config::devices::RTC_PADDR);

    let rtc_base_ptr = phys_to_virt(RTC_PADDR).as_mut_ptr();
