...
impl BootArgs {
    pub(crate) fn saved() -> Self {
        unsafe { core::ptr::read_volatile(&raw const BOOT_ARGS) }
    }
}
...
unsafe extern "C" fn __boot_start() -> ! {
    ...
    _linux_image_header:
        ...
        .globl  _start
    _start:
        # Preserve U-Boot/Linux-style firmware arguments before Rust calls
        # clobber caller-saved registers.
        move        $s0, $a0
        move        $s1, $a1
        move        $s2, $a2

        # DMWIN1: 0x9000... cached RAM/kernel. DMWIN0: 0x8000... uncached MMIO.
        li.d        $t0, {phys_boot_offset} | 0x11
        csrwr       $t0, 0x181
        li.d        $t0, {phys_uncached_offset} | 0x1
        csrwr       $t0, 0x180

        # Jump to cached DMW region
        la.local    $t0, 1f
    ...
    1:
        ...
        csrrd       $a0, 0x20           # cpuid
        li.d        $a1, 0              # DTB paddr is unknown until firmware args are decoded
        la.abs      $t0, {entry}
        li.d        $ra, 0
        jirl        $zero, $t0, 0",

        phys_boot_offset = const PHYS_BOOT_OFFSET,
        phys_uncached_offset = const PHYS_UNCACHED_OFFSET,
